...
## ⚠️ A cor da marca é TERRACOTA, e não indigo

> **Decisão do usuário em 04/09/2026: `#D5623A` substitui o indigo `#6366F1` em todo o site,
> logotipo incluído.** O restante deste documento, e o `design/DESIGN.md` que ele traduz, ainda
> descrevem a paleta indigo/cyan: naquilo que for cor, os dois estão **vencidos por esta seção**.
> O que continua valendo lá é tipografia, forma, espaçamento e os critérios de contraste.
>
> A troca alcançou os tokens do `globals.css`, os quatro SVGs de logotipo, o favicon
> e o gradiente de sete stops, cor a cor.
>
> ### Os dois tokens, e por que são dois
>
> | Token | Claro | Escuro | Para quê |
> | --- | --- | --- | --- |
> | `--color-brand` | `#D5623A` | `#D5623A` | preenchimento, ícone, superfície decorativa, texto **grande** |
> | `--color-brand-text` | `#A24A2C` | `#DB7A58` | texto de corpo, rótulo, selo e chip |
>
> ⚠️ **`#D5623A` puro não serve como texto pequeno nem como preenchimento com texto branco.**
> Medido: 3,56:1 sobre o fundo claro, 3,01:1 sobre o container, 3,73:1 com branco por cima. Os
> três reprovam AA. É a mesma armadilha que o indigo tinha, pelo mesmo motivo, e a razão de
> `--color-brand-text` existir.
>
> ⚠️ **`--color-brand-text` foi calibrado contra o chip, não contra a página.** O pior fundo em
> que ele aparece é o `price-badge` / `price-toggle-note`, cujo fundo é a própria marca a 16% e
> resolve em `rgb(246,228,221)` no claro. Um tom calibrado só contra a página dava 4,33:1 ali e
> reprovava. Medido no navegador com o valor atual: **4,89:1 no chip**, 5,63:1 na página.
>
> ### Onde a cor de marca aparece
>
> | Onde | Token | Uso |
> | --- | --- | --- |
> | `site-footer` | `--color-brand` | o "hub" da assinatura, e o cursor da manchete digitada |
> | `hero` | `--color-brand` | a palavra "hub" na headline |
> | `/precos` (h1) | `--color-brand` | a palavra "surpresa" na headline |
> | `/contato` (h1) | `--color-brand` | a palavra "encurta" na headline |
> | `cta` | `--color-brand` | a palavra "não existe" na headline |
> | `plan-comparison` | `--color-brand` | ícone de recurso incluído |
> | `price-badge`, `price-toggle-note` | `--color-brand-text` sobre marca a 16% | selo e nota |
> | `capsule` ativa | rampa terracota | borda e gradiente da pastilha selecionada |
> | `vs-badge` (arquivado) | `#B35231` | preenchimento com texto branco, ver abaixo |
>
> ⚠️ **Todo uso de `--color-brand` como TEXTO precisa ser grande** (≥24px, ou ≥18,66px em peso
> 700), porque 3,56:1 só passa no limite de texto grande. Conferido no navegador: os seis usos da
> home estão em 68px ou 260px, peso 700.
>
> ⚠️ **Preenchimento que carrega texto branco usa `#B35231`** (5,04:1), e não a cor de marca.
> Mesma decisão que existia para o indigo `#5457EE`.
>
> ### O gradiente do símbolo
>
> Sete stops, do âmbar ao terracota. Ele **não termina em preto**: a rampa indigo terminava no
> midnight `#0B1220`, e manter esse fecho deixava a base do símbolo preta, o que o usuário recusou
> em 04/09/2026.
>
> | Offset | Antes (indigo) | Agora |
> | --- | --- | --- |
> | 0 | `#06B6D4` | `#F5D9A8` |
> | 0.2428 | `#358EE3` | `#EFC183` |
> | 0.3878 | `#5176EB` | `#E79A63` |
> | 0.4480 | `#6366F1` | `#E0834E` |
> | 0.6967 | `#4145A0` | `#DA6F42` |
> | 0.8036 | `#2E3474` | **`#D5623A`** |
> | 0.9665 | `#0B1220` | `#C85832` |
>
> Isso responde a pendência que este documento registrava mais abaixo: o gradiente tem **sete**
> stops no asset, e não seis. O sétimo é a ponta clara, que era o cyan.
>
> ⚠️ **`fill` e `stop-color` são coisas diferentes no logotipo, e não se remapeia os dois juntos.**
> O `fill` é identidade: o "Rook" é preto `#0B1220` e o "Hub" é `#D5623A`. O `stop-color` é a rampa
> acima. Um find-and-replace por valor erra sempre um dos lados, porque `#0B1220` e `#D5623A`
> aparecem nos dois papéis. Ancorar a substituição em `stop-color="…"`.
>
> ### Nomenclatura dos arquivos de marca
>
> O sufixo diz **em que fundo o arquivo se aplica**, e não a cor que ele tem:
>
> | Sufixo | Conteúdo | Usar sobre |
> | --- | --- | --- |
> | `-dark` | "Rook" preto, símbolo em gradiente terracota | fundo **claro** |
> | `-white` | "Rook" e símbolo brancos, "Hub" terracota | fundo **escuro** |
>
> São três peças, cada uma no par completo: `rookhub-full-*` (logotipo inteiro),
> `rookhub-symbol-*` (só a torre) e `rookhub-wordmark-*` (só o texto). Todas em
> `public/images/`. O par `symbol` acumula a função de ícone da aba, declarado em
> `metadata.icons` por `prefers-color-scheme`.
>
> ⚠️ **`app/icon.svg` não existe, e não deve voltar.** A convenção do Next geraria um
> `<link rel="icon">` sem `media`, que competiria com os dois declarados e poderia anular a troca
> por tema.
>
> ### O que sobrou da fase de wireframe
>
> A fase de baixa fidelidade em cinzas, de 27/08/2026, **acabou**. Ficou a base editorial: cinzas
> de superfície, raio único de 6px, superfícies como caixas com borda de 1px, e `.surface-deep`
> redefinindo tokens localmente para seções de fundo próprio. Os cinzas do tema continuam
> levemente frios (matiz ~215) e não acompanharam a virada para o quente; migrá-los é decisão de
> design em aberto, não sobra da troca.
>
> Seguem valendo, e vieram de antes: logotipo de terceiro na prova social entra como máscara CSS
> (`.logo-mark`), a estratégia de duas famílias (Sora/Inter), a escala tipográfica, `tabular-nums`,
> o alvo de toque de 44px e o respeito a `prefers-reduced-motion`.
>
> ⚠️ **A navegação do desktop deixou de ser ilhas flutuantes.** Hoje é uma barra só, que encaixa no
> topo em largura total e se solta ao rolar, virando um retângulo arredondado de 1120px com vidro
> (`desktop-nav.tsx`). O mobile ganhou o mesmo par de estados (`mobile-nav.tsx`), e a folha de
> navegação aberta força o estado encaixado. Mexeu em uma, confira a outra. A altura reservada
> continua sendo o token `--header-h` (68px no mobile, 84px a partir de `md`): nunca escreva o
> número na classe.
...
- **Spectrum Gradient: 6 stops no asset, 7 no texto.** O `DESIGN.md` chama o gradiente de
  ...
  (`public/images/rookhub-full-dark.svg`) e vivem em `--gradient-spectrum`:
